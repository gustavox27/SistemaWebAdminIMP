/*
  # Esquema Unificado Completo - Sistema de Gestión de Impresoras

  Esta migración unifica y consolida todas las migraciones anteriores en un esquema
  limpio y completo con todas las correcciones aplicadas.
  ...
  3. Datos iniciales incluidos para usuarios, operadores, modelos
  4. Plantillas de tickets predefinidas
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE table_name = 'toner_changes' AND column_name = 'updated_at'
  ) THEN
    ALTER TABLE toner_changes ADD COLUMN updated_at timestamptz DEFAULT now();
  END IF;

  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_name = 'tickets' AND column_name = 'copied_at'
  ) THEN
    ALTER TABLE tickets ADD COLUMN copied_at timestamptz;
  END IF;

  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_name = 'tickets' AND column_name = 'moved_to_history'
  ) THEN
    ALTER TABLE tickets ADD COLUMN moved_to_history boolean DEFAULT false;
  END IF;

  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_name = 'tickets' AND column_name = 'history_move_date'
  ) THEN
    ALTER TABLE tickets ADD COLUMN history_move_date timestamptz;
  END IF;
END $$;
...
DROP POLICY IF EXISTS "Permitir acceso público a users" ON users;
DROP POLICY IF EXISTS "Permitir acceso público a operators" ON operators;
DROP POLICY IF EXISTS "Permitir acceso público a toner_models" ON toner_models;
DROP POLICY IF EXISTS "Permitir acceso público a fuser_models" ON fuser_models;
DROP POLICY IF EXISTS "Permitir acceso público a printers" ON printers;
DROP POLICY IF EXISTS "Permitir acceso público a toner_inventory" ON toner_inventory;
DROP POLICY IF EXISTS "Permitir acceso público a toner_orders" ON toner_orders;
DROP POLICY IF EXISTS "Permitir acceso público a toner_changes" ON toner_changes;
DROP POLICY IF EXISTS "Permitir acceso público a toner_loans" ON toner_loans;
DROP POLICY IF EXISTS "Permitir acceso público a empty_toners" ON empty_toners;
DROP POLICY IF EXISTS "Permitir acceso público a printer_fusers" ON printer_fusers;
DROP POLICY IF EXISTS "Permitir acceso público a tickets" ON tickets;
DROP POLICY IF EXISTS "Permitir acceso público a ticket_templates" ON ticket_templates;
DROP POLICY IF EXISTS "Permitir acceso público a app_settings" ON app_settings;
...
-- Insertar usuario responsable por defecto
...
-- Insertar modelos de tóner comunes
...
-- Insertar modelos de fusor comunes
...
-- Insertar plantillas de tickets predefinidas
...
-- Insertar configuración inicial de la aplicación
