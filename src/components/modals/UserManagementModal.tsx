...
import { X, Plus, CreditCard as Edit, Trash2, User, Save, Search, Upload, Download, Building, Monitor, AlertTriangle, CheckCircle, FileText, ChevronDown, Star, ChevronLeft, ChevronRight } from 'lucide-react';
import { useStore } from '../../store/useStore';
import { User as UserType } from '../../types';
import { supabaseService } from '../../services/supabaseService';
import { paginatedSupabaseService } from '../../services/paginatedSupabaseService';
...
export default function UserManagementModal({ isOpen, onClose, onSelect, onUserSelect }: UserManagementModalProps) {
  ...
  const loadUsers = useCallback(async () => {
    ...
    try {
      const result = await paginatedSupabaseService.getUsersPaginated({
  ...
  }), [totalCount, currentPage, totalPages, paginatedUsers.length]);

  const handleSubmit = useCallback(async (e: React.FormEvent) => {
    e.preventDefault();

    if (!formData.name.trim() || !formData.empresa.trim() || !formData.usuarioWindows.trim() || !formData.position) {
    ...
    try {
      ...
      if (editingUser) {
        const updatedUser: UserType = {
          ...
          usuarioWindows: formData.usuarioWindows.toLowerCase().trim()
        } as UserType;
  ...
  }, [formData, editingUser, users, updateUser, addUser, loadUsers]);
  ...
  }, [userToDelete, deleteUser, loadUsers]);

  const handleDeleteAll = useCallback(async () => {
    const toastId = toast.loading('Eliminando usuarios...');
    try {
      const allUsersResult = await supabaseService.getAll<UserType>('users');
      const usersToDelete = allUsersResult.filter(u => u.name !== 'Freddy Moscoso');
      ...
      const remainingUsers = allUsersResult.filter(u => u.name === 'Freddy Moscoso');
  ...
  const handleSetDefault = useCallback(async (userName: string) => {
    try {
      ...
      toast.success(`Usuario predeterminado: ${userName}`);
    } catch (error) {
      console.error('Error:', error);
  ...
  const processExcelImport = useCallback(async () => {
    ...
    try {
      const allUsers = await supabaseService.getAll<UserType>('users');
      ...
      toast.success(
        `✓ ${newUsers.length} usuarios importados. ${skippedCount} omitidos (duplicados)`,
  ...
  return (
    <AnimatePresence>
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
        ...
        >
          <div className="bg-gradient-to-r from-blue-600 to-blue-700 text-white p-6 flex-shrink-0 rounded-t-xl">
            <div className="flex items-center justify-between">
              <div className="flex items-center">
                ...
                <div>
                  ...
                  <p className="text-blue-100 mt-1">Administrar usuarios del sistema</p>
                </div>
              </div>
            ...
            <div className="mt-4 grid grid-cols-4 gap-4">
              ...
              <div className="bg-white bg-opacity-10 p-3 rounded-lg text-center">
                ...
                <div className="text-sm opacity-90">Mostrando</div>
              </div>
              <div className="bg-white bg-opacity-10 p-3 rounded-lg text-center">
            ...
            <div className="flex flex-col lg:flex-row gap-4">
              <div className="flex-1">
                <div className="relative">
                  ...
                  <input
                    type="text"
                    placeholder="Buscar por nombre, empresa, usuario Windows..."
              ...
              <div className="flex gap-2">
                <button
                  ...
                  className="px-4 py-3 bg-green-600 text-white rounded-lg hover:bg-green-700 transition-colors flex items-center gap-2 shadow-sm whitespace-nowrap"
                >
                  <Plus size={18} />
                  Agregar
                </button>
                ...
                <div className="relative">
                  <button
                    ...
                    className="px-4 py-3 border border-gray-300 rounded-lg hover:bg-gray-50 transition-colors flex items-center gap-2 shadow-sm whitespace-nowrap"
                  >
                    Acciones
                    <ChevronDown size={16} />
                  </button>
                  ...
                  {showActionsDropdown && (
                    <div className="absolute right-0 mt-2 w-48 bg-white rounded-lg shadow-lg border border-gray-200 z-10">
                      ...
                      <button
                        ...
                        className="w-full px-4 py-2 text-left hover:bg-red-50 flex items-center gap-2 text-red-600 rounded-b-lg"
                      >
                        <Trash2 size={16} />
                        Eliminar Todos
                      </button>
              ...
              <AnimatePresence>
                {showAddForm && (
                  <motion.div
                    ...
                    className="mb-6 bg-gradient-to-r from-blue-50 to-blue-50 border border-blue-200 rounded-xl p-6 shadow-sm flex-shrink-0 overflow-hidden"
                  >
              ...
              <div className="flex-1 min-h-0 flex flex-col">
                <div className="flex items-center justify-between mb-4 flex-shrink-0">
                  <h3 className="text-xl font-bold text-gray-900">
                    Usuarios ({stats.filtered > 0 ? `${(currentPage - 1) * USERS_PER_PAGE + 1}-${Math.min(currentPage * USERS_PER_PAGE, stats.filtered)} de ${stats.filtered}` : '0'})
                  </h3>
                  {defaultUser && (
          ...
          <ConfirmDialog
            ...
            message={
              <div>
                ...
                <p className="mb-2">
                  Estás a punto de eliminar todos los usuarios excepto "Freddy Moscoso".
                </p>
